...
const messages = {
	...
	imageSelected:
		window.wp?.i18n?.__(
			'Image selected. Review the consent notice to continue.',
			'seatryon-ai-virtual-try-on-for-woocommerce'
		) || 'Image selected. Review the consent notice to continue.',
	optimizing:
		window.wp?.i18n?.__(
	...
	optimizationFailed:
		window.wp?.i18n?.__(
			'We could not optimize this image. Please choose another image.',
			'seatryon-ai-virtual-try-on-for-woocommerce'
		) || 'We could not optimize this image. Please choose another image.',
	variationChanged:
		window.wp?.i18n?.__(
			'The selected variation changed. Please choose your image again.',
			'seatryon-ai-virtual-try-on-for-woocommerce'
		) || 'The selected variation changed. Please choose your image again.',
	imageRemoved:
		window.wp?.i18n?.__(
	...
	cameraReady:
		window.wp?.i18n?.__(
			'Camera ready. Position the photo and press Capture photo.',
			'seatryon-ai-virtual-try-on-for-woocommerce'
		) || 'Camera ready. Position the photo and press Capture photo.',
	cameraUnsupported:
		window.wp?.i18n?.__(
			'Camera capture is not supported in this browser. Please upload a photo instead.',
			'seatryon-ai-virtual-try-on-for-woocommerce'
		) ||
		'Camera capture is not supported in this browser. Please upload a photo instead.',
	...
	cameraFailed:
		window.wp?.i18n?.__(
			'We could not access the camera. Please try again or upload a photo instead.',
			'seatryon-ai-virtual-try-on-for-woocommerce'
		) ||
		'We could not access the camera. Please try again or upload a photo instead.',
	captureFailed:
		window.wp?.i18n?.__(
			'We could not capture this photo. Please try again.',
			'seatryon-ai-virtual-try-on-for-woocommerce'
		) || 'We could not capture this photo. Please try again.',
	submitting:
		window.wp?.i18n?.__(
	...
	processing:
		window.wp?.i18n?.__(
			'Generating your preview. This may take a moment…',
			'seatryon-ai-virtual-try-on-for-woocommerce'
		) || 'Generating your preview. This may take a moment…',
	succeeded:
		window.wp?.i18n?.__(
...
function getConfig( state ) {
	...
	return config &&
		Number( config.productId ) === Number( state.root.dataset.productId )
		? config
		: null;
}
...
async function refreshGuestTokens( config, signal ) {
	...
	const response = await window.fetch( config.guestTokenUrl, {
		...
		body: JSON.stringify( { product_id: Number( config.productId ) } ),
		signal,
	} );
...
function openResultLightbox( state ) {
	...
	const template =
		document.querySelector( '#photoswipe-fullscreen-dialog.pswp' ) ||
...
async function loadResult( state, job, config ) {
	...
	state.download.download = `virtual-try-on-${ job.id }.${
...
async function pollJob( state, job, config ) {
	...
	while ( Date.now() - started < MAX_POLL_DURATION_MS ) {
		...
		if ( document.hidden ) {
			...
			await new Promise( ( resolve, reject ) => {
				...
				const cleanup = () => {
					...
					state.controller.signal.removeEventListener(
						'abort',
						onAbort
					);
				};
		...
		const response = await window.fetch(
			`${ config.restRoot }jobs/${ encodeURIComponent( current.id ) }`,
			{
				credentials: 'same-origin',
...
async function deleteJob( state, jobId = state.jobId, silent = false ) {
	...
	try {
		...
		const response = await window.fetch(
			`${ config.restRoot }jobs/${ encodeURIComponent( jobId ) }`,
			{
				method: 'DELETE',
				credentials: 'same-origin',
...
function updateGenerateState( state ) {
	...
	if ( state.cameraOpen ) {
		state.cameraOpen.disabled =
			state.busy || state.optimizing || state.cameraOpening || completed;
	}
	if ( state.cameraCapture ) {
		state.cameraCapture.disabled =
			state.busy || state.cameraOpening || state.cameraCapturing;
	}
...
function formatFileMeta( file ) {
	const type = ( file.type.split( '/' )[ 1 ] || 'image' )
		.replace( 'jpeg', 'JPG' )
		.replace( 'png', 'PNG' )
		.replace( 'webp', 'WebP' );
...
function closeDialog( state ) {
	...
	state.root.dispatchEvent(
		new CustomEvent( 'sea-tryon:closed', { bubbles: true } )
	);
}
...
function openDialog( state, trigger ) {
	...
	state.dialog.focus();
	state.root.dispatchEvent(
		new CustomEvent( 'sea-tryon:opened', { bubbles: true } )
	);
}
...
function handleDialogKeydown( event, state ) {
	...
	if (
		event.shiftKey &&
		( activeElement === first || activeElement === state.dialog )
	) {
		event.preventDefault();
		last.focus();
...
async function openCamera( state ) {
	...
	try {
		const facingMode =
			state.root.dataset.experienceMode === 'scene'
				? 'environment'
				: 'user';
		...
		showCameraFailure(
			state,
			denied ? messages.cameraPermissionDenied : messages.cameraFailed
		);
	}
}
...
async function handleSelectedFile( state, file ) {
	...
	if ( ! ALLOWED_TYPES.includes( file.type ) ) {
		...
		Math.min(
			MAX_FILE_BYTES,
			Number( getConfig( state )?.maxUploadBytes ) || MAX_FILE_BYTES
		)
	) {
	...
	try {
		...
		const failureMessage =
			optimizationError?.message || messages.optimizationFailed;
		clearFile( state );
		setStatus( state, '' );
 ...
 * @return {void}
 */
export function invalidateForVariation( root, variationId ) {
	...
	state.controller?.abort();
	const staleJobs = [
		...new Set( [ state.jobId, state.resultJobId ].filter( Boolean ) ),
	];
	...
	state.root.dispatchEvent(
		new CustomEvent( 'sea-tryon:variation-invalidated', {
			bubbles: true,
			detail: { variationId: normalized },
		} )
	);
}
 ...
 */
export function setTryOnState( root, nextState, message = '' ) {
	const state = roots.get( root );
	...
	if ( state.progress ) {
		state.progress.setAttribute(
			'aria-hidden',
			state.busy ? 'false' : 'true'
		);
	}
	updateGenerateState( state );
	...
	state.root.dispatchEvent(
		new CustomEvent( 'sea-tryon:state-changed', {
			bubbles: true,
...
function initializeRoot( root ) {
	...
	root.addEventListener( 'keydown', ( event ) =>
		handleDialogKeydown( event, state )
	);
	...
	state.cameraCapture.addEventListener( 'click', () =>
		captureCameraPhoto( state )
	);
	state.cameraCancel.addEventListener( 'click', () => {
		stopCamera( state, true );
		setStatus( state, '' );
	} );
	state.consent.addEventListener( 'change', () =>
		updateGenerateState( state )
	);
	...
	root.querySelector( '[data-sea-tryon-try-again]' )?.addEventListener(
		'click',
		() => {
			state.idempotencyKey = '';
			submitJob( state );
	...
	root.querySelector( '[data-sea-tryon-delete]' )?.addEventListener(
		'click',
		() => deleteJob( state, state.resultJobId || state.jobId )
	);

	root.dispatchEvent(
		new CustomEvent( 'sea-tryon:ready', { bubbles: true } )
	);
}

/** Initialize every newly rendered modal root without duplicate listeners. */
export function initializeTryOnRoots() {
...
document.addEventListener( 'click', ( event ) => {
	...
	const productId = String( trigger.dataset.productId || '' ).replace(
		/[^0-9]/g,
		''
	);
...
if ( window.jQuery ) {
	window
		.jQuery( document )
		.on(
			'found_variation.seaTryOn',
			'form.variations_form',
			( event, variation ) => {
				const productId = event.currentTarget.dataset.product_id;
				const normalized = String( productId || '' ).replace(
					/[^0-9]/g,
					''
				);
				const root = document.querySelector(
					`${ ROOT_SELECTOR }[data-product-id="${ normalized }"]`
				);
	...
	window
		.jQuery( document )
		.on(
			'reset_data.seaTryOn hide_variation.seaTryOn',
			'form.variations_form',
			( event ) => {
				const productId = event.currentTarget.dataset.product_id;
				const normalized = String( productId || '' ).replace(
