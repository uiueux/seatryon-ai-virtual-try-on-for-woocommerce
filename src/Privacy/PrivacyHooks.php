...
final class PrivacyHooks {
	...
	public function add_policy_content(): void {
		...
		$content .= '<p>' . __( 'To prevent anonymous abuse, the plugin derives a one-way HMAC value from the web server-provided client IP address for each guest generation request. It uses that value together with the anonymous session to enforce the configured daily guest limit. The raw IP address is not stored, and the one-day quota record is configured to expire at the next site-local midnight.', 'seatryon-ai-virtual-try-on-for-woocommerce' ) . '</p>';
